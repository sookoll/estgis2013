...
function get_map(the_map, map_name) {
    ...
    ws.onmessage = function (event) {
        ...
        if (event.data instanceof ArrayBuffer) //ArrayBuffer? ok, then it is our geometry comming
        {
            ...
            parse_binary(ta, ws_calls[ta[0]][0], ws_calls[ta[0]][1]);
            //send the binary array for parsing with the mapname, and the map to add the data to as parameters
...
function build(id, the_map, map_name) {
    ...
    if (map_name == "kom_org") { //alert("build:"+geom.geometry.coordinates);
        L.multiPolygon(geom.coordinates, {
            smoothFactor: 1,
            weight: 1,
            color: 'green',
            fillOpacity: 0.01
